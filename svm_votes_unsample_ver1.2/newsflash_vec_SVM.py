...
def train_model(pos_datas, neg_datas, traindatarate, modelpath, modelnumber, C, kernel, shrinking, probability, tol, class_weight, verbose, max_iter):
    ...
    test_neg_datas, remain_neg_datas = pick_datas(neg_datas, test_num)
    ...
    for i in range(modelnumber):
        if len(remain_neg_datas['X'])==0:
            continue

        show_bar('train classifier %d'%(i+1))
        train_neg_datas, remain_neg_datas = pick_datas(remain_neg_datas, train_num)
        ...
        clf.fit(train_datas['X'], train_datas['y'])
        joblib.dump(clf, modelpath+'%d.model'%(i))

        predict_y.append(clf.predict(test_datas['X']))
...
if __name__ == '__main__':
    ...
    parser.add_argument("-modelname", default="svc_linear", help="the model path, save for train and use for test!")
    parser.add_argument("-modelnumber", default=10, type=int, help="the model path, save for train and use for test!")
    parser.add_argument("-traindatarate", default=0.8, type=float, help="the model path, save for train and use for test!")
