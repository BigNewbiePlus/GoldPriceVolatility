...
def create_base_estimators(modelnumber, kernel, verbose):
    ...
    estimators = [('gb', clf1), ('rf', clf2),('lr',clf3),('gnb', clf4)]
...
def train_model(pos_datas, neg_datas, traindatarate, modelpath, modelnumber, C, kernel, shrinking, probability, tol, class_weight, verbose, max_iter):
    ...
    sample_data = 0
    while len(remain_neg_datas['X'])>0:
        sample_data+=1
        show_bar('train sample data %d'%(sample_data))
...
if __name__ == '__main__':
    ...
    parser.add_argument("-modelname", default="voting.model", help="the model path, save for train and use for test!")
